...
export const addTask = (taskName, userId,deadline) => async (dispatch) => {
    ...
    try {
        const response = await fetch('https://project-management-tools-backend.onrender.com/task/add', {
            method: 'POST',
            headers: {
...
export const getAllTasks = (token, id) => async (dispatch) => {
    try {
        ...
        const url = new URL('https://project-management-tools-backend.onrender.com/task/tasks');
        if (id) {
...
export const ArrowClick = (item, direction) => async (dispatch) => {
    ...
    try {
        ...
        const response = await axios.put(`https://project-management-tools-backend.onrender.com/task/${item._id}/status`, taskData);
...
export const deleteItem = (id) => async (dispatch) => {
    try {
        const response = await fetch(`https://project-management-tools-backend.onrender.com/task/${id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
            },
